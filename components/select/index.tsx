...
const StyledSelect = styled.select<Themeable & Pick<SelectProps, 'hasError'>>`
  ...
  background-image:
    ...
    linear-gradient(to right, ${({ theme }) => theme.light50} 0%, ${({ theme }) => theme.light50} 2%, #f3f3f3 3%, #f3f3f3 100%)
  ;
  background-repeat: no-repeat;
  background-position: right 18px top 50%, right 0 top 0;
  background-size: 9px auto, 45px 45px;
  border: solid 1px ${({ hasError, theme }) => (hasError ? theme.error : theme.light50)};
  border-radius: 3px;
  cursor: pointer;
